...
__global__ void transformSurfaceToGlobal ( float* adgpu, size_t pitch, int dim_x, int dim_y) 
/* Function to transform surface memory to a global array (pitched). */
{
		...
		float *row = (float*)((char*)adgpu + idy*pitch);
                // Read from input surface, save into the Global memory
                surf2Dread(&(row[idx]), array_surf, idx*sizeof(float) , idy); 
        }
}

__global__ void  transformReducedSurface ( float* redgpu, int dim_x)
...
__device__ void prop_in_block(float *a, float *b, int thread_id)
...
{
	__shared__ float a_array[REDUCE_BLOCK_SIZE];
	...
	if (block_space < k && idy < n)
        {
		// Check how much is left to reduce
		...
		// Get relevant data
			...
			if (first == 1)
                        {
                                surf2Dread(&a_array[thread_id], array_surf, idx*sizeof(float), idy);
				a_array[thread_id]/=(float)(m);
			}
                        else
                        {
	                        surf2Dread(&a_array[thread_id], reduced_surf, idx*sizeof(float), idy);
			}
                }
		...
		{
			a_array[0] += a_array[1];
			surf2Dwrite(a_array[thread_id], reduced_surf, block_id*sizeof(float), idy);			
		}
	}
}
...
/* Host function to simulate 7 propagations of heat flow through radiator. */
{
	...
	__shared__ float a_array[BLOCK_SIZE];
  	__shared__ float b_array[BLOCK_SIZE];  
	...
	if (return_area < m && idy<n)
 	{
		int xindex = (section_head+thread_id)%m;
  		surf2Dread(&a_array[thread_id], array_surf, xindex*sizeof(float), idy); 	 	
		...
		// Write back to global memory:	
  		...
  		{
			surf2Dwrite(b_array[thread_id], array_surf, (return_area + thread_id -14)*sizeof(float),idy);
  		}
  	}
}
...
/* CPU function which calls necessary kernels to simulate cylindrical radiator.  */
{
  ...
  //Timing of GPU allocation
  cudaEventRecord(start,0);

  cudaMallocPitch((void**)&adgpu,&ag_size, (size_t)(sizeof(float)*A->nColumns), A->nRows);
  ...
  cudaMalloc((void**) &redgpu, sizeof(float)*A->nRows);
  ...
  // Now data transferred to surface memory  
  cudaEventRecord(start, 0);

  cudaMemcpyToArray(cuSurfArray, 0, 0, A->data_, N*sizeof(float), cudaMemcpyHostToDevice);
  ...
  // Timing of Propagation Calculation
  cudaEventRecord(start, 0);
  ...
  //Timing of reduction to get average of each row
  cudaEventRecord(start, 0);
  ...
  cudaMemcpy(av_heats,redgpu,sizeof(float)*A->nRows,cudaMemcpyDeviceToHost);
  cudaMemcpy2D(A->data_, A->nColumns*sizeof(float), adgpu, ag_size, A->nColumns*sizeof(float), A->nRows, cudaMemcpyDeviceToHost);
